...
/*
Creare istanza della classe con le credenziali necessarie (sandbox flag, username, password, api signature).
*/
...
class Paypal
{
    ...
    public function setBNCode($bnCode)
    {
	$this->sBNCode = $bnCode;
    }
    ...
    */
    ...
    public function CallShortcutExpressCheckoutExt( $paymentAmount, $currencyCodeType, $paymentType, $returnURL, $cancelURL,$headerImagePath,$itemName,$NoShipping=true)
    {
            ...
            $nvpstr = $nvpstr . "&PAYMENTREQUEST_0_DESC=" . urlencode($itemName);
            $nvpstr = $nvpstr . "&RETURNURL=" . urlencode($returnURL);
            $nvpstr = $nvpstr . "&CANCELURL=" . urlencode($cancelURL);
            $nvpstr = $nvpstr . "&PAYMENTREQUEST_0_CURRENCYCODE=" . $currencyCodeType;
            $nvpstr = $nvpstr . "&ITEM_NAME=".urlencode($itemName);
            $nvpstr = $nvpstr . "&LOCALECODE=IT";
            $nvpstr = $nvpstr . "&HDRIMG=".urlencode($headerImagePath);
    ...
    '--------------------------------------------------------------------------------------------------------------------------------------------	
    */
    public function CallShortcutExpressCheckout( $paymentAmount, $currencyCodeType, $paymentType, $returnURL, $cancelURL,$NoShipping=true)
    {
            //------------------------------------------------------------------------------------------------------------------------------------
            // Construct the parameter string that describes the SetExpressCheckout API call in the shortcut implementation
    ...
    */
    public function ConfirmPayment( $FinalPaymentAmt )
    {
            ...
            //$nvpstr .= '&PAYMENTREQUEST_0_NOTIFYURL='. urlencode('http://miosito.falcini.com/notify.php');  //TODO parametrizzare
    ...
    */
    private function hash_call($methodName,$nvpStr)
    {
       ...
       //Set proxy name to PROXY_HOST and port number to PROXY_PORT in constants.php 
            ...
            $nvpreq="METHOD=" . urlencode($methodName) . "&VERSION=" . urlencode(PAYPAL_version) . "&PWD=" . urlencode($this->API_Password) . "&USER=" . urlencode($this->API_UserName) . "&SIGNATURE=" . urlencode($this->API_Signature) . $nvpStr . "&BUTTONSOURCE=" . urlencode($this->sBNCode);
